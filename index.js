<script>
    ...
    const getProjectName = (projectId, descriptionText) => {
        ...
        return projectId.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
    };
    ...
    const openModal = async (projectId, discipline, { skipHistory = false, replaceState = false } = {}) => {
        ...
        try {
            ...
            if (images.length === 0) {
                ...
                const projectName = getProjectName(projectId, projectText);
                if (headerTitle) {
    ...
    const loadProjectsGrid = async (discipline) => {
        const manifest = await loadManifest(discipline);
        const projects = Object.keys(manifest).sort();
        ...
        projects.forEach((projectId, index) => {
            const thumbnailPath = findThumbnail(projectId);
            ...
            img.onerror = function() {
                const altPaths = [
                    ...
                    `${getBaseProjectPath(discipline)}${projectId}/thumbnail.png`
                ];
                ...
                const tryNextAlt = () => {
                    if (altIndex < altPaths.length) {
                        ...
                        if (projectAssets.length > 0) {
                            ...
                            // No assets at all, hide the image
